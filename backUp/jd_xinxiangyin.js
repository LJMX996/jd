...
cron 12 9,12,15 1-30 9,10 * jd_xinxiangyin.js
 * */
const $ = new Env('古蜀寻宝');
...
!(async () => {
    ...
    let res = ['df5e2c339fe3437a9d6a9f2da97e9e00'];
    try{res = await getAuthorShareCode('');}catch (e) {}
    if(!res){
        try{res = await getAuthorShareCode('');}catch (e) {}
        if(!res){res = ['df5e2c339fe3437a9d6a9f2da97e9e00'];}
    }
    if (!cookiesArr[0]) {
...
async function main() {
    ...
    if(!$.activityData.opencard){
        if(!$.shareUuid){$.shareUuid = 'df5e2c339fe3437a9d6a9f2da97e9e00';}
        await join($.venderId);
        await $.wait(2000);
        await takePostRequest('activityContent');
        await $.wait(2000);
